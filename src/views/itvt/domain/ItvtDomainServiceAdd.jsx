...
export default function ItvtDomainServiceAdd() {
  ...
  const fetchDomainService = async () => {
    try {
      ...
      toast.error(error.response?.data?.message || 'Có lỗi xảy ra khi lấy thông tin dịch vụ domain ITVT');
      navigate('/itvt/domain');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEdit && !hasPermission(PERMISSIONS.DOMAIN_SERVICE_ITVT.UPDATE)) {
      toast.error('Bạn không có quyền cập nhật dịch vụ tên miền ITVT');
      return;
    }

    if (!isEdit && !hasPermission(PERMISSIONS.DOMAIN_SERVICE_ITVT.ADD)) {
      toast.error('Bạn không có quyền thêm dịch vụ tên miền ITVT mới');
      return;
    }
    ...
    try {
      ...
      if (response?.data?.success) {
        toast.success(isEdit ? 'Cập nhật dịch vụ tên miền ITVT thành công' : 'Thêm dịch vụ tên miền ITVT thành công');
        navigate('/itvt/ten-mien');
      } else {
        toast.error(response?.data?.message || `Có lỗi xảy ra khi ${isEdit ? 'cập nhật' : 'thêm'} dịch vụ tên miền ITVT`);
      }
    } catch (error) {
      console.error('Error:', error);
      toast.error(error.response?.data?.message || `Có lỗi xảy ra khi ${isEdit ? 'cập nhật' : 'thêm'} dịch vụ tên miền ITVT`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
        ...
        <Typography variant="h3">{isEdit ? 'Cập nhật dịch vụ tên miền ITVT' : 'Thêm dịch vụ tên miền ITVT mới'}</Typography>
      </Box>
