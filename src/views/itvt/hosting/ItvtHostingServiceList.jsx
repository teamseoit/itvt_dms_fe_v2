...
export default function ItvtHostingServiceList() {
  ...
  const fetchHostingServices = async (pageNumber = 1) => {
    try {
      ...
      const response = await HOSTING_SERVICE_API.getAll(params);
      if (response.data.success) {
        setData({
          hostingServices: response.data.data,
          meta: response.data.meta
        });
      }
    } catch (error) {
      toast.error(error.response?.data?.message || 'Có lỗi xảy ra khi lấy danh sách dịch vụ hosting ITVT');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAdd = () => {
    navigate('/itvt/hosting/them-moi');
  };

  const handleEdit = (id) => {
    navigate(`/itvt/hosting/${id}`);
  };
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      if (response.data.success) {
        toast.success('Xóa dịch vụ hosting ITVT thành công');
        fetchHostingServices(page + 1);
        fetchCounts();
      }
    } catch (error) {
      toast.error(error.response?.data?.message || 'Có lỗi xảy ra khi xóa dịch vụ hosting ITVT');
    } finally {
      setLoading(false);
      setOpenDialog(false);
      setDeleteId(null);
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Typography variant="h3">Danh sách dịch vụ hosting ITVT</Typography>
        {hasPermission(PERMISSIONS.HOSTING_SERVICE_ITVT.ADD) && (
          <Button
            variant="contained"
            startIcon={<IconPlus />}
            onClick={handleAdd}
      ...
      {hasPermission(PERMISSIONS.HOSTING_SERVICE_ITVT.VIEW) ? (
      <>
      ...
      <Paper sx={{ width: '100%', overflow: 'hidden' }}>
        <TableContainer>
          <Table stickyHeader aria-label="bảng dịch vụ hosting ITVT">
            <TableHead>
              <TableRow>
                {columns.map((column) => (
                  <TableCell
                  ...
                  return (
                    <>
                      <TableRow hover role="checkbox" tabIndex={-1} key={row._id || row.id}>
                        ...
                        <TableCell>
                          {hasPermission(PERMISSIONS.HOSTING_SERVICE_ITVT.UPDATE) && (
                            <IconButton 
                              color="primary" 
                          ...
                          {hasPermission(PERMISSIONS.HOSTING_SERVICE_ITVT.DELETE) && (
                            <IconButton 
                              color="error" 
                          ...
                          <Collapse in={isOpen} timeout="auto" unmountOnExit>
                            <Box sx={{ display: 'flex', flexDirection: 'row', gap: 2 }}>
                              <Box margin={1} sx={{ flex: 1 }}>
                                <Typography variant="subtitle1">Chi tiết dịch vụ hosting ITVT</Typography>
        ...
        <Paper sx={{ p: 3, textAlign: 'center' }}>
          <Typography variant="h6" color="error">
            Bạn không có quyền xem danh sách dịch vụ hosting ITVT
          </Typography>
        </Paper>
      )}
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="dialog-description">
            Bạn có chắc chắn muốn xóa dịch vụ hosting ITVT này không?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
