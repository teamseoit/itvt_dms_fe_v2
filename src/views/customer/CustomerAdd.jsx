...
export default function CustomerAdd() {
  ...
  const fetchUserData = async () => {
    try {
      const response = await CUSTOMER_API.getById(id);
      if (response.data.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = isEdit
        ? await CUSTOMER_API.update(id, userData)
        : await CUSTOMER_API.create(userData);
  ...
  const handleChangePassword = async () => {
    ...
    try {
      setLoading(true);
      const response = await CUSTOMER_API.changePassword(id, newPassword);
      if (response?.data?.success) {
